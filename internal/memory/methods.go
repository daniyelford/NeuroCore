...
func (m Memory) TrySet(index int, value float32) bool {
	if index < 0 ||
		index >= len(m.data) {
		return false
	}
...
func New(size int) Memory {
	return NewOnDevice(size, backend.CPU)
}
func NewOnDevice(size int, dev backend.DeviceType) Memory {
	if size < 0 {
		size = 0
	}
	return Memory{
...
func NewView(m *Memory, offset int, length int) View {
	return View{
		memory: m,
		offset: offset,
		length: length,
...
func From(data []float32) Memory {
	m := NewOnDevice(len(data), backend.CPU)
	copy(m.data, data)
	return m
}
